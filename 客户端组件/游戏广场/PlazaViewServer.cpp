...
void CPlazaViewServer::Notify(TNotifyUI &  msg)
{
	...
	if (lstrcmp(msg.sType, TEXT("click")) == 0)
	{
		...
		{
			CMatchViewFrame * pViewFrame = (CMatchViewFrame *)m_pITableViewFrame;
			if(pViewFrame->m_cbUserMatchStatus == MUS_SIGNUPED &&  pViewFrame->m_cbMatchStatus <   MS_MATCHING)
			{
				//if(	pViewFrame->m_GameMatch.cbMatchType)
				{
					//提示消息
					CDlgInformation Information(this);
					INT nRes=0;
					if (m_wServerType==GAME_GENRE_MATCH)
						nRes=Information.ShowMessageBox(TEXT("退出房间将退出比赛，确认退出吗？"),MB_ICONQUESTION|MB_YESNO,0);
...
bool CPlazaViewServer::OnProcessManagerData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	//返回大厅
	if ((wMainCmdID==IPC_CMD_GF_CONTROL)&&(wSubCmdID==IPC_SUB_GF_BACK_PLAZA))
	{
		//关闭进程
		m_ProcessManager.CloseGameProcess(true);

		CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
		if (pPlatformFrame!=NULL) 
		{
			pPlatformFrame->ReturnHallMain();
			pPlatformFrame->SetActiveWindow();
			//pPlatformFrame->BringWindowToTop();
			pPlatformFrame->SetForegroundWindow();
		}
		return true;
	}
