...
bool CDlgIndividual::EfficacyModifyLogonPassward()
{
	...
	if (lstrcmp(szDesPassword[1],szDesPassword[0])==0)
	{
		//提示信息
		CDlgInformation Information(this);
		Information.ShowMessageBox(TEXT("新密码与旧密码一样，无需修改！"),MB_ICONERROR,0);

		return false;
	}

	//帐号相同
	if (lstrcmp(szDesPassword[1],pGlobalUserData->szAccounts)==0)
	{
		//提示信息
		CDlgInformation Information(this);
		Information.ShowMessageBox(TEXT("新密码与帐号不能一样，请重新输入！"),MB_ICONERROR,0);
...
bool CDlgIndividual::EfficacyModifyInsurePassward()
{
	...
	if (lstrcmp(szDesPassword[1],pGlobalUserData->szAccounts)==0)
	{
		//提示信息
		CDlgInformation Information(this);
		Information.ShowMessageBox(TEXT("新密码不能与帐号相同，请重新输入！"),MB_ICONERROR,0);
	...
	if (lstrcmp(szEncryptPassword,pGlobalUserData->szPassword)==0)
	{
		//提示信息
		CDlgInformation Information(this);
		Information.ShowMessageBox(TEXT("新密码不能与登录密码相同，请重新输入！"),MB_ICONERROR,0);
