...
VOID CDlgServerManager::OnOK()
{
	...
	//允许回放
	CButton * pSupportPlayBack=(CButton *)GetDlgItem(IDC_SUPPORT_PLAY_BACK);
	CServerRule::SetSuportPlayBack(ServerOption.ServerOptionInfo.dwServerRule,(pSupportPlayBack->GetCheck()==BST_CHECKED));
...
VOID CDlgServerManager::SetServerOptionInfo(tagServerOptionInfo & ServerOptionInfo, DWORD dwRuleMask)
{
	...
	//支持回放
	bool bSupportPlayBack=CServerRule::IsSuportPlayBack(ServerOptionInfo.dwServerRule);
	((CButton *)GetDlgItem(IDC_SUPPORT_PLAY_BACK))->EnableWindow(CServerRule::IsSuportPlayBack(dwRuleMask));
	((CButton *)GetDlgItem(IDC_SUPPORT_PLAY_BACK))->SetCheck((bSupportPlayBack)?BST_CHECKED:BST_UNCHECKED);
