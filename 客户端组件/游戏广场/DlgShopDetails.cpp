...
CDlgShopDetails::CDlgShopDetails() : CFGuiDialog(IDD_DLG_POPUP)
{
	...
	m_pEditBuyAmount	= NULL;

	m_dwBuyNumber = 1;
	m_dwOldBuyNumber = m_dwBuyNumber;
...
void CDlgShopDetails::Notify(TNotifyUI &  msg)
{
 	...
 	if (lstrcmp(msg.sType, TEXT("click")) == 0)
	{
		...
		{			
			m_dwBuyNumber = (WORD)StrToInt(m_pEditBuyAmount->GetText().GetData());
			
			CButtonUI * pButtonUIDec = static_cast<CButtonUI *>(GetControlByName(szButtonDecrease));
			if(pButtonUIDec != NULL) m_dwBuyNumber += pButtonUIDec->GetBindNumberSetp();
			...
			m_pEditBuyAmount->SetText(szText);	
		...
		{			
			...
			CButtonUI * pButtonUIIns = static_cast<CButtonUI *>(GetControlByName(szButtonIncrease));
			if(pButtonUIIns != NULL) m_dwBuyNumber += pButtonUIIns->GetBindNumberSetp();			

			CButtonUI * pButtonUI = static_cast<CButtonUI *>(GetControlByName(szButtonDecrease));
			if(pButtonUI != NULL) pButtonUI->SetEnabled(true);

			if(m_dwBuyNumber >= 999)
			{
				m_dwBuyNumber = 999;
			}	
			//购买数量
	...
	{
		if(lstrcmp(pControlUI->GetName(),szNumberBuy)==0 )
		{
			m_dwBuyNumber = (WORD)StrToInt(m_pEditBuyAmount->GetText().GetData());
			
			if(m_dwBuyNumber != m_dwOldBuyNumber)
			{
				UpdateUI();
				m_dwOldBuyNumber = m_dwBuyNumber;
			}
...
VOID CDlgShopDetails::InitUI()
{
	...
	m_pEditBuyAmount = static_cast<CEditUI*>(GetControlByName(szNumberBuy));
	...
	TCHAR szTaskRewardTotal[128]=TEXT("");
...
VOID CDlgShopDetails::UpdateUI()
{
	...
	ASSERT(pLablePrice != NULL && pLabelVIPPrice != NULL && pLabelBuyScore != NULL && m_pEditBuyAmount != NULL);
	if(pLablePrice == NULL || pLabelVIPPrice == NULL || pLabelBuyScore == NULL || m_pEditBuyAmount == NULL) return;
