		...
		case SUB_GP_USER_INSURE_INFO:		//银行资料
			{
				...
				m_UserInsureInfo.dwUserRight=pUserInsureInfo->dwUserRight;
		...
		case SUB_GP_USER_INSURE_SUCCESS:	//银行成功
			{
				...
				m_pItemPresent->ResetControlVal();
				//获取对象
				CGlobalUserInfo * pGlobalUserInfo = CGlobalUserInfo::GetInstance();
				tagGlobalUserData * pGlobalUserData = pGlobalUserInfo->GetGlobalUserData();
...
void CDlgInsure::ShowPresent()
{
	...
	if (m_pContainerPresent)
	{
		...
		if((m_UserInsureInfo.cbEnjoinTransfer==1)&&(CUserRight::CanOpenTransfer(m_UserInsureInfo.dwUserRight)==false))
		{
			m_pContainerPresent->SetVisible(1);
		}
	}
...
//重置控件值
VOID CInsureItemPresent::ResetControlVal()
{
	//设置焦点
